...
class SberbankczPipeline:

    # Database setup
    conn = sqlite3.connect('sberbankcz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sberbankcz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sberbankcz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sberbankcz (date, title, link, content)"
